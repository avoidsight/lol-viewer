...
打包前先设置客户端真实版本（示例 `0.1.0`，需与后台版本管理填写的一致）：

```sh
node scripts/release-version.mjs set 0.1.0
```

一键打包会提示输入版本号，回车沿用当前版本；`0.0.0` 禁止发布。也可通过 PowerShell 脚本的 `-Version 0.1.0` 参数指定。
版本写入 `apps/desktop/package.json`，应随代码一起提交；安装包文件名、反馈和设备统计均读取该版本。仅修改后台版本或重命名 EXE 不会更新包内版本，已安装的旧包需要重新打包并覆盖安装。
