...
export default function PlayerCard({ player, displayLane = player.lane, uncertain = false }: { player: PlayerSnapshot; displayLane?: keyof typeof laneNames; uncertain?: boolean }) {
  return (
    <article className="player-card" data-testid="player-card" data-lane={displayLane} aria-labelledby={`player-${player.playerId}`}>
      <header className="player-card__header"><span className="player-card__lane">{laneNames[displayLane]}</span><h3 id={`player-${player.playerId}`}>{player.displayName}</h3><span>{player.rank ?? '段位未知'}</span>{uncertain && <span className="player-card__uncertain">位置待确认</span>}</header>
