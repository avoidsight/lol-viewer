  ...
  return (
    <li className={`recent-match ${match.win ? 'recent-match--win' : 'recent-match--loss'}`} data-testid="recent-match">
      {imageUnavailable ? <span className="recent-match__fallback" role="img" aria-label={`${championLabel}图标不可用`}>{match.championId}</span> :
        <img className="recent-match__champion" src={`https://raw.communitydragon.org/latest/plugins/rcp-be-lol-game-data/global/default/v1/champion-icons/${match.championId}.png`} alt={championLabel} loading="lazy" onError={() => setImageUnavailable(true)} />}
