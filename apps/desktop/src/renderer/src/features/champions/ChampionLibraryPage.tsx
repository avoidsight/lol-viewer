...
export default function ChampionLibraryPage({ getGuide }: { getGuide: (id: number, lane: ChampionLane) => Promise<ChampionGuide> }) {
  ...
  if (failed) return <main className="champion-library"><p role="alert">英雄数据暂不可用</p></main>;
  if (!guide) return <main className="champion-library"><p role="status">正在加载英雄资料…</p></main>;
  ...
  return <main className="champion-library"><header><h1>英雄资料库</h1></header>{guide.stale && <strong className="stale">离线缓存</strong>}<p><strong>{sourceNames[guide.source]}</strong> · {guide.region} · {guide.tier} · {guide.patch}</p><p>抓取时间：{new Date(guide.fetchedAt).toLocaleString('zh-CN')}</p><section><h2>推荐出装</h2><ul>{guide.builds.map((x, i) => <li key={i}>{x.itemIds.join(' → ')}{x.pickRate === undefined ? '' : ` · ${(x.pickRate * 100).toFixed(1)}%`}</li>)}</ul></section>{matchup('优势对位', guide.favorable)}{matchup('劣势对位', guide.unfavorable)}<section><h2>管理员说明</h2><ul>{guide.notes.map(x => <li key={x}>{x}</li>)}</ul></section></main>;
}
