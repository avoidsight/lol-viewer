...
it('only offers newer important versions with a valid official download path', async () => {
  for (const value of [null, { ...release, important: false }, { ...release, version: '1.0.0' }, { ...release, version: '0.9.0' }, { ...release, downloadUrl: 'https://evil.test/file.exe' }, { ...release, downloadUrl: '/downloads/../evil' }]) {
...
it('deduplicates requests and throttles checks, but revalidates explicit download', async () => {
  ...
  expect(await Promise.all([service.check(), service.check()])).toEqual([{ version: '1.1.0', notes: release.notes }, { version: '1.1.0', notes: release.notes }]);
