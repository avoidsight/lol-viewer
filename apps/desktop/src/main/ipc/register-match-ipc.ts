...
import { MATCH_GET_CHANNEL, MATCH_RETRY_CHANNEL, PLAYER_UPDATED_CHANNEL, liveMatchRequestSchema, liveMatchSchema, playerUpdateSchema, type LiveMatch } from '../../shared/ipc';
...
export function registerMatchIpc(service: LiveMatchLoader): void {
  ...
  ipcMain.handle(MATCH_RETRY_CHANNEL, (event: IpcMainInvokeEvent) => {
    assertAuthorizedRenderer(event);
    service.retry?.();
  });
}
